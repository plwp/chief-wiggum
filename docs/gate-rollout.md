...
| **Boot-and-hit** | `check_boot_and_hit.py` | `--gate` | **NEW — report-only** (#352); wired report-only into `/close-epic` (Step 2j4) and `/implement-wave`'s wave integration check (item 4, replacing a "if services can be started… verify health endpoints respond" line whose two conditionals are what let the wiring bug through). Probes every non-`external` operation in `contracts.json` against a RUNNING instance; reaching the base URL is itself the startup-panic check. Two deliberate abstentions, neither a finding: mutating methods are not probed without `--probe-mutating` (firing DELETE at a live service destroys data — a gate that damages what it inspects is worse than no gate), and parameterized paths are `unprobeable` without `--path-param` (a 404 from an invented id means "no such record" as readily as "not registered", the exact question being asked). No `--base-url` is `inapplicable`, never `pass`. The entrypoint rule is the CONJUNCTION #352 asked for — no tests AND no boot coverage. No workflow passes `--gate` until a `docs/quality/validation/check_boot_and_hit.json` record exists |
| **Behavioural eval** | `check_behavioral_eval.py` | `--gate` | **NEW — report-only** (#354); wired report-only into `/close-epic` (Step 2j5), for products shipping an agent with tools. Reads the target's `docs/quality/behavioral-evals.json` golden set plus its RESULTS and asserts the outcome — the expected tool was actually called — not that config was well-formed. A pass with no recorded `called_tools` is `unverified`, never `verified`: exit status is not behaviour. A declared tool with no case is `no_case`. Deliberately ships NO lint for the sibling finding (a semantically circular schema — `get_date_info` requiring an `isoDate` to tell you today's date): the plausible heuristic is the same unmeasured prose-intent guess that was rejected for #350's hedge lint, and there is no corpus of real tool schemas to measure its precision against. No workflow passes `--gate` until a `docs/quality/validation/check_behavioral_eval.json` record exists |
| **Fixture provenance** | `check_fixture_provenance.py` | `--gate` | **NEW — report-only** (#351); wired report-only into `/close-epic` (Step 2j6). A test double for a declared external system must be derived from a real captured interaction — `@cw-fixture <system> capture=<path>`, the fourth tag in the `@cw-*` family — and the capture must carry `captured_at`/`source`, so an invented file cannot satisfy the check as well as a real one (the obvious evasion). `no_fixture` is REPORTED and never a finding: not every external system needs a double, and inventing that requirement would be noise. Companion recorder `scripts/record_capture.py` stamps the provenance and records request header values as `<not recorded>` — every one of them, not just the obviously-secret names, since guessing which custom header carries a credential is the guess that leaks one. Cannot tell whether a capture is FAITHFUL (it may be edited afterwards) — that is `check_external_smoke.py`'s question. No workflow passes `--gate` until a `docs/quality/validation/check_fixture_provenance.json` record exists |
