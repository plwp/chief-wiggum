...
**Fold in adopted-pattern clusters (do not re-derive).** For each adopted pattern this epic realizes (from Step 1's `--list-adopted`), add its invariant cluster under a dedicated subsection, **keeping the pattern's own stable ids verbatim** (`INV-<ABBR>-NNN` — e.g. `INV-FOWR-004`, `INV-EAS-003`). These are the pattern's contract; re-numbering them or paraphrasing away their meaning breaks the link back to the contract pack and the ratchet. Every shipped pattern-manifest id already satisfies the same three-segment `KIND-SLUG-NNN` grammar as above (`scripts/check_patterns.py`'s own linter enforces this on `patterns/*/manifest.json`, chief-wiggum#294) — a pattern is never a source of a shape this verbatim-copy step could make unparseable. Cite the source so the provenance is legible:
